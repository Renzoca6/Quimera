...
function chat.Open(mode)
	...
	chatbox.bteam = mode
	chatbox.text_entry:SetPlaceholder((mode ~= 1 and "(TEAM) " or "") .. "type something...")
...
do
	...
	local line_DoRightClick = function(s)
		...
		d_menu:AddOption("Copy Message", function()
		...
		if steamid then
			...
			d_menu:AddOption("Copy Steamid64", function()
				SetClipboardText(steamid)
			end)

			d_menu:AddOption("Show Player Profile", function()
		...
		d_menu:AddSpacer()
		local time = s.time
		d_menu:AddOption("Copy Time", function()
